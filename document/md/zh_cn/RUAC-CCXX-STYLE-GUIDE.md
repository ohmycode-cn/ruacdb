...
- 每组内部按 Aa~Zz 首字母顺序排列
- Aa~Zz 含义：大小写不敏感的字母序，同字母时大写在前、小写在后（即 A, a, B, b, C, c ... Z, z）

### 3. 宏条件头文件规则

- 宏条件头文件放在最后
- 每个分支内部仍按「自定义在上、标准库在下、各自 Aa~Zz」排列

### 4. 平台宏规则

- 平台判断宏中，Linux 为 `#if` 正分支（第一个分支）
- 其他平台（Windows 等）放在 `#else` / `#elif` 分支

### 5. 适用范围

- 以上规则对 `.h` / `.hpp` / `.cpp` / `.c` 等所有文件统一生效
- Include guard（`#ifndef` / `#define` / `#endif`）不属于宏条件头文件，不参与第 3 组
